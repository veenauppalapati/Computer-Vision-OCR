...
#First load the files correctly with this function
def load_img_with_json(path="Input/W2_Single_Clean_jpg/", start=0, stop=46):
    ...
    image_file_names = []
    #The start and stop are manufally input
    jsons = []
    images = []
...
def sanity_test():
    ...
    queries = ["What is the Employee's Address?", "What is the Employee's first name and last name?", "What are the wages, tips and compensation?"]
    json_collect = ["Employee's address", "Employee's first name and last name", "Wages"]
    questions = []
    answers = []
    expecteds = []
    ...
    for idx, img in enumerate(images):
        for idxq, query in enumerate(queries):
            result = question_result(query, img, True)
            expected = jsons[idx][json_collect[idxq]]
            questions.append(query)
            answers.append(result)
            expecteds.append(expected)
